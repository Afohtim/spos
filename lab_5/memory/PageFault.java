...
public class PageFault {
  ...
  public static WorkingSet workingSet;

  private static Page getPageFromVirtualMemory(Vector mem, int id) {
    return ( Page ) mem.elementAt(id);
  }

  private static void resetPage(Page page) {
    page.inMemTime = 0;
  ...
  private static Page replacePages(Vector mem, ControlPanel controlPanel, Page pageToRemove, int replacePageNum) {
    Page pageToInsert = getPageFromVirtualMemory(mem, replacePageNum);

    controlPanel.removePhysicalPage( pageToRemove.id );
    pageToInsert.physical = pageToRemove.physical;
    controlPanel.addPhysicalPage( pageToInsert.physical , replacePageNum);
    resetPage(pageToRemove);
  ...
  {
    Page pageToRemove = workingSet.getPageToRemove();

    Page insertedPage = replacePages(mem, controlPanel, pageToRemove, replacePageNum);

    workingSet.addPage(insertedPage);
