...
public class Kernel extends Thread
{
  ...
  public void init( String commands , String config )  
  {
    ...
    runs = 0;
    Vector<Integer> currentPhysicalPages = new Vector<Integer>();
    for (i = 0; i <= virtPageNum; i++) 
    {
      Page page = (Page) memVector.elementAt(i);
      if ( page.physical != -1 )
      {
        map_count++;
        currentPhysicalPages.add(page.physical);
      }
    ...
    if ( map_count < ( virtPageNum +1 ) / 2 )
    {
      for (i = 0; i <= virtPageNum; i++) 
      {
        ...
        {
          while (currentPhysicalPages.contains(map_count)) {
            map_count++;
          }
          map_count++;
        }
    ...
    PageFault.workingSet = new WorkingSet(memVector, virtPageNum + 1);
  } 
